...
let sensorSocket;
let rotationQuat = [1, 0, 0, 0]; // w, x, y, z
...
function getRotationMatrixFromVector(q) {
    const [w, x, y, z] = q;
...
function connectToSensorServer() {
    const WS_URL = "ws://192.168.0.103:8080/sensor/connect?type=android.sensor.gyroscope";
    sensorSocket = new WebSocket(WS_URL);

    sensorSocket.onopen = () => {
        console.log("Підключено до Sensor Server (gyroscope)");
    };
    ...
    sensorSocket.onmessage = (event) => {
        try {
            ...
            if (Array.isArray(data.values) && data.values.length >= 3 && data.timestamp) {
                const [x, y, z] = [data.values[0], data.values[1], -data.values[2]]; // рад/сек
                const ts = data.timestamp / 1e9; // наносекунди → секунди

                if (lastTimestamp !== null) {
                    const dt = ts - lastTimestamp;

                    const omega = Math.hypot(x, y, z);
                    if (omega > 1e-5) {
                        const axis = [x / omega, y / omega, z / omega];
                        const angle = omega * dt;

                        const deltaQuat = axisAngleToQuaternion(axis, angle);
                        rotationQuat = multiplyQuaternions(rotationQuat, deltaQuat);
                        rotationQuat = normalizeQuaternion(rotationQuat);
                        rotationMatrix = getRotationMatrixFromVector(rotationQuat);
                    }
                }
            ...
            console.error("Error parsing gyroscope data:", e, event.data);
        }
    };
